#ifndef MAPFIT_ESTEP_PHASEPOI_GROUP_GPH
#define MAPFIT_ESTEP_PHASEPOI_GROUP_GPH
...
struct GPHWorkSpaceGroupPoi {
  ...
  GPHWorkSpaceGroupPoi(int m, int n) :
...
double llf(
  ...
  for (int k=1; k<=m; k++) {
    ...
    if (IDAT(k) == 1) {
      // vb[k] = (-ph.T) * barvb[k]
      gemv(NOTRANS{}, -1.0, model.gph.Q, barvb1, 0.0, tmpvb);
      double tmp = dot(model.gph.alpha, tmpvb);
      llf += log(tmp);
      nn += 1;
    }
  ...
  if (gdatlast >= 0) {
    double tmp = dot(model.gph.alpha, barvb1);
...
double estep(
  ...
  return estep_grouppoi(model, baralpha, data, eres, options, work);
}
